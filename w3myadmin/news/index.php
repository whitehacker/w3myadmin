<html>
  <head>
    
  </head>
  
  
  <body>
    <?php
    DEFINE ('host','localhost');
    DEFINE ('user','root');
    DEFINE ('pass','');
    mysql_connect(host,user,pass);
    ini_set("display_errors",1);
    error_reporting(E_ALL ^ E_DEPRECATED);
    echo "<link rel='stylesheet' href='../css/bootstrap.css' />";
    mysql_select_db("isd");
    $selectthree=mysql_query("SELECT * FROM news WHERE is_published=1 order by pub_date desc limit 3");
    $selectall=mysql_query("SELECT * FROM news WHERE is_published=1 order by pub_date desc");
  

    ?>
    <div class="container-fluid">
        <div class="row">
          <div class="col-md-8">
            <?php
            while($row=mysql_fetch_array($selectthree)){
              $txt=$row['text'];
              $txt=substr($txt,0,100);
              $txt.="...";
              ?>
              
              <h3><a href="details.php?id=<?php echo $row['id']; ?>"><?php echo $row['title']; ?></a></h3>
              <span class="text-muted">Published By:<?php echo $row['author']; ?></span>
              
              <span class="text-muted">Published Date:<?php echo $row['pub_date']; ?></span>
              <span class="text-muted">Category:<a href="category.php?cat=<?php echo $row['category']; ?>"><?php echo $row['category']; ?></a></span>
              
              <p>
                <?php echo $txt; ?>
                <a href="details.php?id=<?php echo $row['id']; ?>">Keep Reading</a>
              </p>
            
            <?php
            }
            
            ?>
          </div>
          
          <div class="col-md-4">
            <h3>Recent News</h3>
            <?php
            while($row_all=mysql_fetch_array($selectall)){
              $txt=$row_all['text'];
              $txt=substr($txt,0,100);
              $txt.="...";
              ?>
              
              <a href="details.php?id=<?php echo $row_all['id']; ?>"><?php echo $row_all['title']; ?></a>
              <br/>
              
              
            
            
            <?php
            }
            
            ?>
          </div>
          
        </div>
      
    </div>
    
  </body>

</html>